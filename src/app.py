...
def render_public_deal_page(analysis_id: str) -> None:
    ...
    Triggered by `?deal=<analysis_id>` in the URL. Does not run the password
    gate, does not show the admin sidebar, and exits via `st.stop()` before
    any admin code runs.
    """
    st.set_page_config(
    ...
    if not analysis:
        st.title("Deal not found")
        st.write(
            "The link is invalid, or this analysis no longer exists. "
            "Check with whoever sent you the link."
        )
        return
    ...
    _render_triage_body(analysis)

    st.divider()
    created_at = (analysis.get("created_at") or "")[:19].replace("T", " ")
    st.caption(f"Generated by {firm_name} Thesis Assistant — {created_at}")
