...
Two routes share the same script:
  - **Admin** (default): firm setup, document upload, profile generation,
    deal analysis. Gated by APP_PASSWORD if set.
  - **Public deal page** (`?deal=<analysis_id>`): read-only verdict +
    memo for one analysis. No login, no admin chrome. Anyone with the
    UUID can open it. Documented as a v1 tradeoff in README.
...
from src.views.public_deal import render_public_deal_page as _render_public_deal_page_v2
...
# ----- shared renderers (used by both admin and public deal page) -------------


def _verdict_badge(verdict: str) -> None:
    """Render the verdict alert with appropriate severity color."""
...
def _render_triage_body(analysis: dict) -> None:
    """Render the verdict + bullets + questions + full-memo expander.
    Shared between admin's `render_triage` and the public deal page."""
    _verdict_badge(analysis.get("verdict", "Unknown"))
    ...
    full_memo = analysis.get("full_memo_md", "")
    if full_memo:
        with st.expander("Full memo — deeper analysis", expanded=False):
            st.markdown(full_memo)


def _share_url_for(analysis_id: str) -> str:
    """Compose the public share URL for a given analysis."""
    base = os.environ.get("APP_PUBLIC_URL", "http://localhost:8501").rstrip("/")
    return f"{base}/?deal={analysis_id}"


# The public deal page is now rendered by src.views.public_deal — kept
# this module as a thin wrapper so the route-detour code below stays clean.
def render_public_deal_page(analysis_id: str) -> None:
    _render_public_deal_page_v2(analysis_id)


# ----- public deal page detour ------------------------------------------------
# Must run BEFORE the admin's set_page_config and password gate.
...
def run_deal_analysis_streamed(
    ...
    """Run analysis with live streaming. Saves to session_state."""
    chunks: list[str] = []
...
def render_triage(result: dict, deck_filename: str | None) -> None:
    """Render the admin triage view: shared body + token caption + download +
    copy-share-link."""
    _render_triage_body(result)
    ...
    if analysis_id:
        ...
        st.code(_share_url_for(analysis_id), language=None)
